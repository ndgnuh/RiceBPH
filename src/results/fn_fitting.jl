...
function qcd(x)
```
The `qcd` function calculates the coefficient of dispersion (CD) for a given dataset `x` using the interquartile range (IQR).
...
The `qcd` function calculates the CD by first computing the first quartile `q1` and the third quartile `q3` using the `quantile` function from the `Statistics` module. The `quantile` function returns the value that corresponds to the given quantile of the dataset. In this case, `q1` represents the 25th percentile (Q1), and `q3` represents the 75th percentile (Q3).
...
### Note
- The `quantile` function used in the `qcd` function assumes the default values for the `beta` and `alpha` parameters, which correspond to the definitions of the quartiles used in traditional statistics. However, these parameters can be adjusted if desired.
- The `qcd` function assumes that the input dataset `x` is numeric. Providing non-numeric values may result in unexpected behavior or errors.
"""
function qcd(x)
...
function fit_fn(
   ...
   results = combine(groups) do group
      ...
      param_part = (
         name => value for
         (name, value) in zip(param_names, fit.param)
      )
...
function compute_stats(fit_df, group_key; only_converged=true)
   ...
   stat_fns = [maximum, minimum, mean, std, qcd]
