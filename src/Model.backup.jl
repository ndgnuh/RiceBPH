...
function agent_step!(agent, model)
    ...
    if (agent.age ≥ model.age_init && agent.energy ≥ model.energy_move) && (
        ...
        all_directions = model.move_directions[agent.isshortwing]
        directions = filter(all_directions) do (dx, dy)
...
function model_step!(model)
    ...
    for (idx, agent) in model.agents
        if agent.age < model.age_init
            model.num_eggs += 1
        elseif agent.age < model.age_reproduce
            model.num_nymphs += 1
    ...
    num_adults = model.num_nymphs + model.num_brachys + model.num_macros
    model.r_nymphs = model.num_nymphs / num_adults
    model.r_macros = model.num_macros / num_adults
    model.r_brachys = model.num_brachys / num_adults
    num_agents = length(model.agents)
    model.grow_rate = num_agents / model.previous_num_agents
    model.previous_num_agents = num_agents
...
const r_nymph_ma = stream_moving_average(Float32, 24)
const num_eggs_ma = stream_moving_average(Float32, 24)
const MODEL_DATA = let
    [num_healthy_rice, :grow_rate, :r_nymphs]
end
