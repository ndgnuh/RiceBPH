...
"""
    init_flower_mask(map_size::Integer, flower_width::Integer)

Returns a bool matrix, 1 ~ food cell, 0 ~ flower cell.
"""
function init_flower_mask(map_size::Integer, flower_width::Integer)
    flower_mask = ones(Bool, (map_size, map_size))
...
"""
    init_pr_eliminate(init_pr::Float32, flower_mask::Matrix{Bool}; gauss::Float32 = 2.5f0)
    init_pr_eliminate(P::Float32, M::Matrix{Bool}; σ::Float32 = 2.5f0)

Returns Float32 matrix of elimination probability.
Matrix is generated by ``(P ⋅ (1 - M)) ∗ G(σ)``.
"""
function init_pr_eliminate(init_pr::Float32, flower_mask::Matrix{Bool};
                           gauss::Float32 = 2.5f0)
    kernel = Kernel.gaussian(gauss)
    pr_eliminate = imfilter((@. init_pr * !flower_mask), kernel)
    return pr_eliminate
end
