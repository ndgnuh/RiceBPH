...
for rep in 1:max_replications
   # Run simulation
   model::Agents.AgentBasedModel = RiceBPH.Models.init_model(
      params; rng
   )
   df::DataFrame = RiceBPH.Models.run_ricebph!(model)

   # Save collected simulation data
   output::String = joinpath(
      OUTPUT_PATH, @sprintf "replication-%05i" rep
   )
   JDF.save(output, df)
   ...
   let showvalues = [
         ...
         (
            :init_pr_eliminate,
            string(params.init_pr_eliminate),
         ),
