...
# ╔═╡ 0ba40ae8-6af1-11ed-3167-f5b5694ccd63
using Zygote

# ╔═╡ 06c41983-01bc-47b2-8a5b-ca6aa540e07e
using Flux

# ╔═╡ b992b744-e1f2-4799-afeb-1b80e8a4d733
using Agents

# ╔═╡ 87f1ee8e-4e73-407a-9c20-cd9d2b9f8b4a
using DataFrames

# ╔═╡ 2d3ca78a-6ccb-40c8-a040-c6211dfda62a
using JLD2

# ╔═╡ 4eb8b7c7-b34d-4d5d-991c-cfe06d160f81
using Plots

# ╔═╡ 813814af-8759-43e9-8461-0882977d0b0e
using SplitApplyCombine

# ╔═╡ 98feda16-0034-441b-a4bd-61ec9c3b16ab
using StatsBase

# ╔═╡ cb8650a1-83e2-41d4-8c10-5cdb99966309
module Models
...
function agent_step!(agent, model)
    ...
    for neighbor in nearby_agents(agent, model, trunc(Int, model.radius))
...
function initialize(; numagents = 320, radius=1f0, griddims = (20, 20), min_to_be_happy = 3, seed = nothing)
    space = GridSpaceSingle(griddims, periodic = false)
    properties = Dict(:min_to_be_happy => min_to_be_happy, :radius => radius)
...
# ╔═╡ 92456f71-ab27-4751-8067-eb83db4f4624
io = load("../result/envmap=020-1x2.csv-init_nb_bph=200-init_position=border-init_pr_eliminate=0.075.jld2")

# ╔═╡ 6dbca288-8fdf-4662-8358-4ac732866555
results = io["results"];

# ╔═╡ 17448add-23c2-4a15-8f2f-c313ab9f6bc9
begin
	struct ExponentialLayer
		params::Vector{Float32} 
	end
	function (m::ExponentialLayer)(t)
		# https://en.wikipedia.org/wiki/Generalised_logistic_function
		A, K, B, ν, Q, C = m.params
		result = @. A + (K - A) / (C + Q * exp(-ν * t)^(1/ν))
		# L, k, t0 = m.params
		# @. L / (1 + exp(-k * (t - t0)))
		# @. m.params[1] * sin(m.params[3] * t) + m.params[2]
	end
	Flux.@functor ExponentialLayer
end

# ╔═╡ e605cdc8-a2d5-469f-968e-f10965cb5568
begin
	pexp = Chain(
		ExponentialLayer(randn(Float32, 9)),
	)
	function loss(x, y)
		Flux.Losses.mse(pexp(x), y)
	end
end

# ╔═╡ 04c3c4d2-7556-4d94-bff1-1f07313caca5
pexp

# ╔═╡ 515c3cd7-1b2a-4720-818d-023cb1c98f6d
optimizer = Flux.Optimise.AdamW(1f-3)

# ╔═╡ 67c46a83-ee42-479d-9441-85488f07df4c
plot(1:2881, pexp(1:2881))

# ╔═╡ a0d1949c-d9db-48f4-8807-c4352c9f5a81
num_healthy_rice = [
	result[!, :num_healthy_rice]
	for result in results
]

# ╔═╡ 4a0bda86-33db-47eb-9d91-16d10414d286
data = [
	begin
		x = 1:2881
		y = d .- mean(d) ./ std(d)
	...
	for d in num_healthy_rice[1:100]
]

# ╔═╡ 03ffbf9b-33ad-4461-bbb0-4959ee646241
for e in 1:10
	Flux.Optimise.train!(loss, Flux.params(pexp), data, optimizer)
end

# ╔═╡ 3f8812ed-b9b7-4d31-a907-5e8d08b2e1f7
let p = plot()
	for y in eachcol(num_healthy_rice[1:100])
		plot!(1:2881, y)
	end
	p
end

# ╔═╡ 562060ee-56d4-4811-928a-6a3c6caa5d88


# ╔═╡ a872c0f2-08b3-404b-a0f0-436647bb5ac9
function init_and_run()
end

# ╔═╡ 664513ec-77e0-41c5-ae9c-a562971916d5
function is_happy(agent)
	agent.mood
end

# ╔═╡ 4f95f534-3e7d-4bd5-87dd-c0427a953117
function run_get_result(radius; steps=100)
	model = Models.initialize(; radius=radius * 1f0)
	adata = [(is_happy, count)]
	step!(model, Models.agent_step!, Agents.dummystep, 10)
	return count(is_happy, model.agents)
end

# ╔═╡ ec7efb24-4031-4124-9eaa-0dac999f57bb
gradient(run_get_result, 2)

# ╔═╡ 403ff45a-a06e-499a-b64b-c19034ea24fe
run_get_result(1f0)
...
manifest_format = "2.0"
project_hash = "32403ad4703a21681373f6acf58a1bd6b05c376d"
...
git-tree-sha1 = "627238422f2fbda490a42e2984a834f6914d85cf"
